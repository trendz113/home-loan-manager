...
| `/admin/rates?token=...` | GET/POST | View and edit lender rates in a simple web form (token-protected) — updates take effect immediately, no redeploy |
...
- `RATES_FILE_PATH` — where `rates.json` is written, e.g. `/data/rates.json` (same Volume as leads — see below)
- `ADMIN_TOKEN` — long random secret that protects the `/admin/leads` and `/admin/rates` routes
- `REPORT_PRICE_PAISE` — optional, defaults to `14900` (₹149)

## Updating lender rates (no redeploy needed)

There's no reliable free API for live Indian bank lending rates, and scraping
bank websites directly is fragile and often against their terms of service —
not something to build a payment product on. Floating home loan rates mainly
move after RBI's roughly bimonthly MPC meetings, so a manual-but-easy update
process is the realistic, sustainable approach here.

Visit:

```
https://<your-railway-url>/admin/rates?token=<your ADMIN_TOKEN value>
```

This shows an editable table of every lender and rate. Change any values,
add new lenders in the blank rows, and click **Save Rates** — the change
takes effect on the very next calculation, immediately, with no code change
or redeploy.

**Important:** `RATES_FILE_PATH` must point at the same Railway Volume as
`LEADS_FILE_PATH` (e.g. `/data/rates.json`) or your rate edits will be lost
on the next redeploy, just like leads would be without a volume.

**Suggested cadence:** check and update after each RBI MPC announcement
(roughly every 2 months), or whenever a DSA partner flags a rate change on
their end.
