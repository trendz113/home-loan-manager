...
# Answers the question the EMI calculator above deliberately doesn't:
# "how much EXTRA do I need to pay every month to close this loan in
# 5 / 6 / 7 / 10 years instead of running the full remaining tenure?"
# Pure guaranteed-savings math — no investment return assumptions mixed in.

def total_interest_for_tenure(principal: float, annual_rate: float, months: int) -> float:
    r = (annual_rate / 12) / 100
    if r == 0:
...
def amortize_with_extra(principal, annual_rate, base_emi, extra_monthly=0.0,
                         annual_lumpsum=0.0, lumpsum_month=12, max_months=600):
    """Month-by-month simulation with an extra fixed monthly prepayment and/or
    an annual lump-sum prepayment. Returns (months_to_close, total_interest_paid)."""
    r = (annual_rate / 12) / 100
    balance = principal
    month = 0
    total_interest = 0.0
    ...
    while balance > 0 and month < max_months:
        ...
        if annual_lumpsum > 0 and month % lumpsum_month == 0:
            balance -= min(annual_lumpsum, max(balance, 0))
...
def required_extra_for_target(principal, annual_rate, remaining_months,
                               target_months, annual_lumpsum=0.0):
    """Binary-searches for the extra monthly payment needed to close the loan
    in target_months, given an optional annual lump sum stacked on top."""
    base_emi = calculate_emi(principal, annual_rate, remaining_months / 12)

    lo, hi = 0.0, principal
    for _ in range(60):
        mid = (lo + hi) / 2
        months, _ = amortize_with_extra(
            principal, annual_rate, base_emi, extra_monthly=mid,
    ...
    extra = round(hi, -1)  # round to nearest ₹10 for a clean number
    months, total_interest = amortize_with_extra(
        principal, annual_rate, base_emi, extra_monthly=extra,
        annual_lumpsum=annual_lumpsum
    )
    return {
        "extra_monthly_required": extra,
        "new_total_emi": round(base_emi + extra, 2),
        "actual_closure_months": months,
        "actual_closure_years": round(months / 12, 1) if months else None,
        "total_interest_paid": total_interest,
    }


@app.route("/api/loan-closure-planner", methods=["POST"])
def api_loan_closure_planner():
    data = request.get_json(silent=True) or {}
    try:
        principal = float(data.get("loan_amount", 0))
        annual_rate = float(data.get("current_rate", 0))
        remaining_months = int(round(float(data.get("tenure_years", 0)) * 12))
    except (TypeError, ValueError):
        return jsonify({"error": "Invalid input. Please send numeric values."}), 400

    if principal <= 0 or annual_rate <= 0 or remaining_months <= 0:
        return jsonify({"error": "loan_amount, current_rate, and tenure_years must all be positive numbers."}), 400

    annual_lumpsum = float(data.get("annual_lumpsum", 0) or 0)
    target_years = data.get("target_years") or [5, 6, 7, 10]

    base_emi = round(calculate_emi(principal, annual_rate, remaining_months / 12), 2)
    baseline_total_interest = round(total_interest_for_tenure(principal, annual_rate, remaining_months), 2)

    plans = []
    for years in target_years:
        target_months = years * 12
        if target_months >= remaining_months:
            continue  # not actually an early closure
        plan = required_extra_for_target(principal, annual_rate, remaining_months, target_months, annual_lumpsum)
        if plan["actual_closure_months"] is None:
            continue
        plan["interest_saved_vs_baseline"] = round(baseline_total_interest - plan["total_interest_paid"], 2)
        plan["target_years"] = years
        plans.append(plan)
    ...
    return jsonify({
        "baseline": {
            ...
            "total_interest_if_full_tenure": baseline_total_interest,
        },
        "annual_lumpsum_assumed": annual_lumpsum,
        "closure_plans": plans,
    })
